...
* Reddcoin Core 2.0 (based on Bitcoin Core 0.9) or later
...
* python send-OP_RETURN.py <send-address> <send-amount> <metadata> <testnet (optional)>
  ...
  <send-amount> is the amount to send (in units of RDD)
...
* Wait a minute then check for your OP_RETURN transaction.
...
* OP_RETURN_send(send_address, send_amount, metadata, testnet=False)
  ...
  send_amount is the amount to send (in units of RDD)
...
* Wait a minute then check for your OP_RETURN transactions.
